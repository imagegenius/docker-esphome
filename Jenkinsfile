    ...
    stage("Set ENV Variables base"){
      steps{
        script{
          env.EXIT_STATUS = ''
          env.RELEASE = sh(
            script: '''docker run --rm ghcr.io/linuxserver/alexeiled-skopeo sh -c 'skopeo inspect docker://docker.io/'${DOCKERHUB_IMAGE}':ubuntu 2>/dev/null' | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-' || : ''',
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-ubuntu-' + env.EXT_RELEASE_CLEAN + '-' + env.TAG_NUMBER + '|arm32v7-ubuntu-' + env.EXT_RELEASE_CLEAN + '-' + env.TAG_NUMBER + '|arm64v8-ubuntu-' + env.EXT_RELEASE_CLEAN + '-' + env.TAG_NUMBER
          } else {
            env.CI_TAGS = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-' + env.TAG_NUMBER
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-' + env.TAG_NUMBER
          env.META_TAG = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-' + env.TAG_NUMBER
