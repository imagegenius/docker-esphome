pipeline {
  ...
  environment {
    ...
    DOCKERHUB_IMAGE = 'hydaz/esphome'
    DEV_DOCKERHUB_IMAGE = 'dev/esphome'
    PR_DOCKERHUB_IMAGE = 'pipepr/esphome'
    DIST_IMAGE = 'ubuntu'
    MULTIARCH = 'false'
    CI = 'true'
    CI_WEB = 'true'
  ...
  stages {
    ...
    stage("Set ENV Variables base"){
      steps{
        script{
          env.EXIT_STATUS = ''
          env.RELEASE = sh(
            script: '''docker run --rm ghcr.io/linuxserver/alexeiled-skopeo sh -c 'skopeo inspect docker://docker.io/'${DOCKERHUB_IMAGE}':ubuntu 2>/dev/null' | jq -r '.Labels.build_version' | awk '{print $2}' | grep '\\-hz' || : ''',
    ...
    // If this is a ubuntu build use live docker endpoints
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-ubuntu-' + env.EXT_RELEASE_CLEAN + '-hz' + env.TAG_NUMBER + '|arm32v7-ubuntu-' + env.EXT_RELEASE_CLEAN + '-hz' + env.TAG_NUMBER + '|arm64v8-ubuntu-' + env.EXT_RELEASE_CLEAN + '-hz' + env.TAG_NUMBER
          } else {
            env.CI_TAGS = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-hz' + env.TAG_NUMBER
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-hz' + env.TAG_NUMBER
          env.META_TAG = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-hz' + env.TAG_NUMBER
          env.EXT_RELEASE_TAG = 'ubuntu-version-' + env.EXT_RELEASE_CLEAN
        }
      }
    }
    ...
    stage("Set ENV dev build"){
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA + '|arm32v7-ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA + '|arm64v8-ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          } else {
            env.CI_TAGS = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          env.META_TAG = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-dev-' + env.COMMIT_SHA
          env.EXT_RELEASE_TAG = 'ubuntu-version-' + env.EXT_RELEASE_CLEAN
    ...
    stage("Set ENV PR build"){
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST + '|arm32v7-ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST + '|arm64v8-ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          } else {
            env.CI_TAGS = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          }
          env.VERSION_TAG = env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          env.META_TAG = 'ubuntu-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          env.EXT_RELEASE_TAG = 'ubuntu-version-' + env.EXT_RELEASE_CLEAN
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=ubuntu -v ${TEMPDIR}:/ansible/jenkins ghcr.io/hydazz/jenkins-builder:latest 
              # Stage 1 - Jenkinsfile update
              if [[ "$(md5sum Jenkinsfile | awk '{ print $1 }')" != "$(md5sum ${TEMPDIR}/docker-${CONTAINER_NAME}/Jenkinsfile | awk '{ print $1 }')" ]]; then
                mkdir -p ${TEMPDIR}/repo
                git clone https://github.com/${USER}/${REPO}.git ${TEMPDIR}/repo/${REPO}
                cd ${TEMPDIR}/repo/${REPO}
                git checkout -f ubuntu
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/hydazz/docker-esphome/packages\" \
          --label \"org.opencontainers.image.source=https://github.com/hydazz/docker-esphome\" \
          ...
          --label \"org.opencontainers.image.description=[ESPHome](https://esphome.io/) - A system to control your ESP8266/ESP32 by simple yet powerful configuration files and control them remotely through Home Automation systems.  \" \
      ...
      steps {
        sh '''#! /bin/bash
              ...
              if [ "${NEW_PACKAGE_TAG}" != "${PACKAGE_TAG}" ]; then
                ...
                git --git-dir ${TEMPDIR}/${REPO}/.git checkout -f ubuntu
        ...
        withCredentials([
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITHUBIMAGE}"; do
                    ...
                    docker tag ${MANIFESTIMAGE}:amd64-${META_TAG} ${MANIFESTIMAGE}:amd64-ubuntu
                    docker tag ${MANIFESTIMAGE}:arm32v7-${META_TAG} ${MANIFESTIMAGE}:arm32v7-ubuntu
                    docker tag ${MANIFESTIMAGE}:arm64v8-${META_TAG} ${MANIFESTIMAGE}:arm64v8-ubuntu
                    ...
                    docker push ${MANIFESTIMAGE}:amd64-ubuntu
                    docker push ${MANIFESTIMAGE}:arm32v7-ubuntu
                    docker push ${MANIFESTIMAGE}:arm64v8-ubuntu
                    ...
                    docker manifest push --purge ${MANIFESTIMAGE}:ubuntu || :
                    docker manifest create ${MANIFESTIMAGE}:ubuntu ${MANIFESTIMAGE}:amd64-ubuntu ${MANIFESTIMAGE}:arm32v7-ubuntu ${MANIFESTIMAGE}:arm64v8-ubuntu
                    docker manifest annotate ${MANIFESTIMAGE}:ubuntu ${MANIFESTIMAGE}:arm32v7-ubuntu --os linux --arch arm
                    docker manifest annotate ${MANIFESTIMAGE}:ubuntu ${MANIFESTIMAGE}:arm64v8-ubuntu --os linux --arch arm64 --variant v8
          ...
          sh '''#! /bin/bash
                for DELETEIMAGE in "${GITHUBIMAGE}" "${IMAGE}"; do
                  ...
                  ${DELETEIMAGE}:amd64-ubuntu \
                  ${DELETEIMAGE}:amd64-${EXT_RELEASE_TAG} \
                  ${DELETEIMAGE}:arm32v7-${META_TAG} \
                  ${DELETEIMAGE}:arm32v7-ubuntu \
                  ${DELETEIMAGE}:arm32v7-${EXT_RELEASE_TAG} \
                  ${DELETEIMAGE}:arm64v8-${META_TAG} \
                  ${DELETEIMAGE}:arm64v8-ubuntu \
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${META_TAG}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-hz'${TAG_NUMBER}' to ubuntu",\
             "type": "commit",\
